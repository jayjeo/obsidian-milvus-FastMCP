...
def search_by_filename(query, limit=10, show_details=False):
    ...
    try:
        ...
        # 컬렉션 로드 시도 (이미 로드되어 있어도 안전함)
        try:
            print(f"{Fore.YELLOW}컬렉션을 로드합니다...{Fore.RESET}")
            manager.collection.load()
        except Exception as e:
            # 이미 로드되어 있거나 다른 오류가 발생한 경우 무시
            print(f"{Fore.YELLOW}참고: {e}{Fore.RESET}")
        
        # Milvus는 중간 와일드카드를 지원하지 않으므로 모든 문서를 가져와서 클라이언트 측에서 필터링
        results = manager.collection.query(
            expr="chunk_index >= 0",  # 모든 문서 가져오기
            output_fields=["id", "path", "chunk_index", "title", "content"],
        ...
        for result in results:
            ...
            if query.lower() in path.lower():  # 대소문자 구분 없이 검색
