...
class EnhancedSearchEngine(SearchEngine):
    ...
    def advanced_filter_search(self, query, **filters):
        ...
        filter_expr = " && ".join(filter_expressions) if filter_expressions else None
        
        # Milvus HNSW 인덱스를 활용한 빠른 검색
        ...
        results = self.milvus_manager.search(
            ...
            search_params={
                "metric_type": "COSINE",
                "params": {"ef": 128}  # HNSW 성능 최적화
            }
        )
        
        return self._format_enhanced_results(results, query)
    
    def semantic_similarity_search(self, query, similarity_threshold=0.8):
        """의미적 유사도 기반 고급 검색"""
        # 1단계: 기본 벡터 검색
        query_vector = self.embedding_model.get_embedding(query)
        initial_results = self.milvus_manager.search(
            ...
            search_params={
                "metric_type": "COSINE",
                "params": {"ef": 256}  # 높은 정확도를 위한 HNSW 설정
            }
        )
        ...
        for hit in initial_results:
            if hit.score >= similarity_threshold:
                # 추가 의미 분석
                semantic_score = hit.score  # 기본적으로 Milvus 점수 사용
                formatted_result = {
                    "id": hit.id,
        ...
        # 3단계: 의미적 점수로 재정렬
        filtered_results.sort(key=lambda x: x['semantic_score'], reverse=True)
        
        return filtered_results
    
    def contextual_search(self, query, context_docs=None, expand_context=True):
        """컨텍스트 기반 확장 검색 (RAG 최적화)"""
    ...
    def multi_modal_search(self, query, include_attachments=True):
        ...
        if include_attachments:
            ...
            attachment_results = self.milvus_manager.search(
                vector=self.embedding_model.get_embedding(query),
                limit=20,
            ...
            for hit in attachment_results:
                ...
                formatted_attachment_results.append(formatted_result)
    ...
    def _build_complex_tag_filter(self, tag_logic):
        ...
        # 예: {"and": ["work", "project"], "or": ["urgent", "important"], "not": ["archived"]}
        expressions = []
        
        if tag_logic.get('and'):
            and_expressions = []
            for tag in tag_logic['and']:
                # Milvus에서 배열 필드 검색을 위한 JSON 함수 사용
                and_expressions.append(f"json_contains(tags, '\"{tag}\"')")
            if and_expressions:
        ...
        if tag_logic.get('or'):
            ...
            for tag in tag_logic['or']:
                or_expressions.append(f"json_contains(tags, '\"{tag}\"')")
            if or_expressions:
        ...
        if tag_logic.get('not'):
            ...
            for tag in tag_logic['not']:
                not_expressions.append(f"!json_contains(tags, '\"{tag}\"')")
            if not_expressions:
        ...
        for hit in all_results:
            ...
            if doc_id not in result_map:
                result_map[doc_id] = {
                    ...
                    "created_at": hit.entity.get('created_at', ''),
                    "updated_at": hit.entity.get('updated_at', ''),
                    "context_scores": [hit.score]
                }
            else:
                result_map[doc_id]["context_scores"].append(hit.score)
        
        # 컨텍스트 점수 계산 (평균 + 빈도 보너스)
    ...
    def _expand_context_chunks(self, results):
        ...
        for result in results[:10]:  # 상위 10개 결과만 확장
            # 현재 결과 추가
    ...
    def _merge_multimodal_results(self, text_results, attachment_results):
        """다중 모달 결과 병합"""
        # 텍스트 결과에 높은 가중치, 첨부파일 결과에 중간 가중치
